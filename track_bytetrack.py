...
# ByteTrack-like parameters (simplified)
TRACK_THRESH = 0.5       # detections above this start / keep tracks
MATCH_THRESH = 0.7       # IoU threshold for association
INACTIVE_TTL = 30        # how many frames we keep a lost track before deleting
...
class Track:
    ...
    score: float
    last_frame: int
    inactive: int = 0       # number of frames since last update
...
class ByteTrackerSimple:
    ...
    def update(self, frame_idx: int, bboxes: np.ndarray, scores: np.ndarray):
        ...
        Args:
            frame_idx: int
            bboxes: Nx4 ndarray (x1, y1, x2, y2)
            scores: N ndarray (conf)
        Returns:
            list of active Track after this frame
        """
        # Step 1: mark all tracks as inactive by +1 (we will reset to 0 upon match)
        for t in self.tracks:
            t.inactive += 1

        if bboxes is None or len(bboxes) == 0:
            # No detections: just prune tracks
            self._cleanup()
            return self.tracks
        ...
        if M == 0:
            # No existing tracks: create new ones from high-threshold detections
            for i in range(N):
        ...
        for ti, trk in enumerate(self.tracks):
            for di in range(N):
                iou_mat[ti, di] = iou(trk.bbox, bboxes[di])

        # Step 3: greedy matching (simplified instead of Hungarian for clarity)
        unmatched_tracks = set(range(M))
        unmatched_dets = set(range(N))

        matches = []
        # repeatedly pick the best (track, det) pair with IoU >= MATCH_THRESH
        while True:
        ...
        # Step 5: unmatched detections -> potential new tracks
        for di in unmatched_dets:
            if scores[di] >= self.track_thresh:
                self._start_new_track(frame_idx, bboxes[di], scores[di])

        # Step 6: cleanup tracks that are dead too long
        self._cleanup()
...
def find_yolo_detections(root: Path, date_filter: str | None = None):
    """
    Yield paths to yolo_detections.csv under processed root.
    """
...
def run_bytetrack_on_video(yolo_csv: Path):
    """
    yolo_csv: path to yolo_detections.csv of a single video.
    Produces bytetrack_tracks.csv with columns:
    ...
    if out_csv.exists():
        # Avoid re-running unless you delete the file
        return out_csv
    ...
    for _, row in df.iterrows():
        ...
        # Save active track boxes for this frame
        for t in tracks:
            # only output if this track was updated on this frame
...
def main():
    ap = argparse.ArgumentParser(description="Run simple ByteTrack-style tracking over YOLO detections.")
    ap.add_argument("--root", type=str, default=DEFAULT_ROOT,
                    help="Processed root containing date folders (e.g., FIT622_processed)")
    ...
    print(f"[INFO] Found {len(yolo_files)} yolo_detections.csv files under {root}"
          + (f" (date={args.date})" if args.date else ""))

    done, skipped = 0, 0
    for csv_path in tqdm(yolo_files, desc="ByteTrack", unit="video"):
        out_csv = run_bytetrack_on_video(csv_path)
        if out_csv is None:
            skipped += 1
        else:
            done += 1

    print(f"[DONE] ByteTrack finished for {done} video(s); skipped {skipped} (already had bytetrack_tracks.csv).")
